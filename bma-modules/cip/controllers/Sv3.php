...
class Sv3 extends BMA_Controller {

    function __construct() {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv3'));
    ...
    function index() {
        ...
        $data['sv3s'] = $this->db->select('*')->get('sv3s')->result();
        $this->template->title('SV 1');
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv3/index_v', $data);
    }
    ...
    public function create()
    {
        ...
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv3/create_v');
    }
    ...
    public function store()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $store = $this->db->insert('sv3s', $data);
            redirect(base_url('cip/sv3/'));
        }
        else{
    ...
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv3show'));
        parent::__construct($config);

        if($id != NULL){
            $sv3 = $this->db->get_where('sv3s', array('id'=>$id ))->result();
            $data['sv3'] = $sv3;
            $this->template->title('Show');
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv3/show_v', $data);
        }
    }

    public function edit($id = NULL)
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv3edit'));
        parent::__construct($config);
        if($id != NULL){
            $sv3 = $this->db->get_where('sv3s', array('id'=>$id ))->result();
            $data['sv3'] = $sv3;
            ...
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv3/edit_v', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            //Wipe and Swab Test
                ...
                $data['swab_test'] = isset($postData['swab_test']) ? $data['swab_test'] = TRUE : $data['swab_test'] = FALSE;
            $this->db->where('id', $id);
            $update = $this->db->update('sv3s', $data);
            // print_r($update);
            // exit();
            redirect(base_url('cip/sv3/'));
        }
