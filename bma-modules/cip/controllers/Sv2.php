...
class Sv2 extends BMA_Controller {

    function __construct() {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv2'));
    ...
    function index() {
        ...
        $data['sv2s'] = $this->db->select('*')->get('sv2s')->result();
        $this->template->title('SV 1');
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv2/index_v', $data);
    }
    ...
    public function create()
    {
        ...
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv2/create_v');
    }
    ...
    public function store()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $store = $this->db->insert('sv2s', $data);
            redirect(base_url('cip/sv2/'));
        }
        else{
    ...
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv2show'));
        parent::__construct($config);

        if($id != NULL){
            $sv2 = $this->db->get_where('sv2s', array('id'=>$id ))->result();
            $data['sv2'] = $sv2;
            $this->template->title('Show');
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv2/show_v', $data);
        }
    }

    public function edit($id = NULL)
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv2edit'));
        parent::__construct($config);
        if($id != NULL){
            $sv2 = $this->db->get_where('sv2s', array('id'=>$id ))->result();
            $data['sv2'] = $sv2;
            ...
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv2/edit_v', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            //Wipe and Swab Test
                ...
                $data['swab_test'] = isset($postData['swab_test']) ? $data['swab_test'] = TRUE : $data['swab_test'] = FALSE;
            $this->db->where('id', $id);
            $update = $this->db->update('sv2s', $data);
            // print_r($update);
            // exit();
            redirect(base_url('cip/sv2/'));
        }
