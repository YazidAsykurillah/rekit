...
class Sv4tohp4 extends BMA_Controller {

    function __construct() {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv4tohp4'));
    ...
    function index() {
        ...
        $data['sv4tohp4s'] = $this->db->select('*')->get('sv4tohp4s')->result();
        $this->template->title('SV4 to HP4');
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv4tohp4/index_v', $data);
    }
    ...
    public function create()
    {
        ...
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv4tohp4/create_v');
    }
    ...
    public function store()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $store = $this->db->insert('sv4tohp4s', $data);
            redirect(base_url('cip/sv4tohp4/'));
        }
        else{
    ...
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv4tohp4show'));
        parent::__construct($config);

        if($id != NULL){
            $sv4tohp4 = $this->db->get_where('sv4tohp4s', array('id'=>$id ))->result();
            $data['sv4tohp4'] = $sv4tohp4;
            $this->template->title('Show');
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv4tohp4/show_v', $data);
        }
    }

    public function edit($id = NULL)
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv4tohp4edit'));
        parent::__construct($config);
        if($id != NULL){
            $sv4tohp4 = $this->db->get_where('sv4tohp4s', array('id'=>$id ))->result();
            $data['sv4tohp4'] = $sv4tohp4;
            ...
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv4tohp4/edit_v', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $data['updated_at'] = date('Y-m-d H:i:s');
            $this->db->where('id', $id);
            $update = $this->db->update('sv4tohp4s', $data);
            // print_r($update);
            // exit();
            redirect(base_url('cip/sv4tohp4/'));
        }
