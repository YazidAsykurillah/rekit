...
class Sv1tohp1 extends BMA_Controller {

    function __construct() {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv1tohp1'));
    ...
    function index() {
        ...
        $data['sv1tohp1s'] = $this->db->select('*')->get('sv1tohp1s')->result();
        $this->template->title('SV1 to HP1');
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv1tohp1/index_v', $data);
    }
    ...
    public function create()
    {
        ...
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv1tohp1/create_v');
    }
    ...
    public function store()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $store = $this->db->insert('sv1tohp1s', $data);
            redirect(base_url('cip/sv1tohp1/'));
        }
        else{
    ...
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv1tohp1show'));
        parent::__construct($config);

        if($id != NULL){
            $sv1tohp1 = $this->db->get_where('sv1tohp1s', array('id'=>$id ))->result();
            $data['sv1tohp1'] = $sv1tohp1;
            $this->template->title('Show');
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv1tohp1/show_v', $data);
        }
    }

    public function edit($id = NULL)
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv1tohp1edit'));
        parent::__construct($config);
        if($id != NULL){
            $sv1tohp1 = $this->db->get_where('sv1tohp1s', array('id'=>$id ))->result();
            $data['sv1tohp1'] = $sv1tohp1;
            ...
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv1tohp1/edit_v', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $data['updated_at'] = date('Y-m-d H:i:s');
            $this->db->where('id', $id);
            $update = $this->db->update('sv1tohp1s', $data);
            // print_r($update);
            // exit();
            redirect(base_url('cip/sv1tohp1/'));
        }
