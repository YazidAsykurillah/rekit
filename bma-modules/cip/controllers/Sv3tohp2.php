...
class Sv3tohp2 extends BMA_Controller {

    function __construct() {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv3tohp2'));
    ...
    function index() {
        ...
        $data['sv3tohp2s'] = $this->db->select('*')->get('sv3tohp2s')->result();
        $this->template->title('SV3 to HP2');
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv3tohp2/index_v', $data);
    }
    ...
    public function create()
    {
        ...
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv3tohp2/create_v');
    }
    ...
    public function store()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $store = $this->db->insert('sv3tohp2s', $data);
            redirect(base_url('cip/sv3tohp2/'));
        }
        else{
    ...
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv3tohp2show'));
        parent::__construct($config);

        if($id != NULL){
            $sv3tohp2 = $this->db->get_where('sv3tohp2s', array('id'=>$id ))->result();
            $data['sv3tohp2'] = $sv3tohp2;
            $this->template->title('Show');
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv3tohp2/show_v', $data);
        }
    }

    public function edit($id = NULL)
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv3tohp2edit'));
        parent::__construct($config);
        if($id != NULL){
            $sv3tohp2 = $this->db->get_where('sv3tohp2s', array('id'=>$id ))->result();
            $data['sv3tohp2'] = $sv3tohp2;
            ...
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv3tohp2/edit_v', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $data['updated_at'] = date('Y-m-d H:i:s');
            $this->db->where('id', $id);
            $update = $this->db->update('sv3tohp2s', $data);
            // print_r($update);
            // exit();
            redirect(base_url('cip/sv3tohp2/'));
        }
