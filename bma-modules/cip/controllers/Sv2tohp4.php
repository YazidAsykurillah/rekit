...
class Sv2tohp4 extends BMA_Controller {

    function __construct() {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv2tohp4'));
    ...
    function index() {
        ...
        $data['sv2tohp4s'] = $this->db->select('*')->get('sv2tohp4s')->result();
        $this->template->title('SV2 to HP4');
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv2tohp4/index_v', $data);
    }
    ...
    public function create()
    {
        ...
        $this->template->set('icon', 'fa fa-gears');
        $this->template->build('cip/sv2tohp4/create_v');
    }
    ...
    public function store()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $store = $this->db->insert('sv2tohp4s', $data);
            
            redirect(base_url('cip/sv2tohp4/'));
        }
        else{
    ...
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv2tohp4show'));
        parent::__construct($config);

        if($id != NULL){
            $sv2tohp4 = $this->db->get_where('sv2tohp4s', array('id'=>$id ))->result();
            $data['sv2tohp4'] = $sv2tohp4;
            $this->template->title('Show');
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv2tohp4/show_v', $data);
        }
    }

    public function edit($id = NULL)
    {
        $config = array('modules' => 'cip', 'jsfiles' => array('sv2tohp4edit'));
        parent::__construct($config);
        if($id != NULL){
            $sv2tohp4 = $this->db->get_where('sv2tohp4s', array('id'=>$id ))->result();
            $data['sv2tohp4'] = $sv2tohp4;
            ...
            $this->template->set('icon', 'fa fa-gears');
            $this->template->build('cip/sv2tohp4/edit_v', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if($postData['recipe_name'] != ""){
            ...
            $data['updated_at'] = date('Y-m-d H:i:s');
            $this->db->where('id', $id);
            $update = $this->db->update('sv2tohp4s', $data);
            // print_r($update);
            // exit();
            redirect(base_url('cip/sv2tohp4/'));
        }
