(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('master/dataset_intropowder/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
                ...
                //--- Insert
                $.post('master/dataset_intropowder/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'master/dataset_intropowder/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTbma(t, 'master/dataset_intropowder/delete');
