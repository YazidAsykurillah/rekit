(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            ...
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('master/tank_process/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
                ...
                //--- Insert
                $.post('master/tank_process/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
    ...
    $('#mainTable').on('click', 'a.btnedit', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#tankname").val(d.tankname).focus();
        $("#tankcapacity").val(d.tankcapacity);
        $("#tankorder").val(d.tankorder);
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'master/tank_process/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTbma(t, 'master/tank_process/delete');
