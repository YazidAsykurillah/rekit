(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            ...
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('master/unit_material/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
                ...
                //--- Insert
                $.post('master/unit_material/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'master/unit_material/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTbma(t, 'master/unit_material/delete');
