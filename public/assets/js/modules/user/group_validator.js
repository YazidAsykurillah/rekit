(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            ...
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('user/group_validator/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
                ...
                //--- Insert
                $.post('user/group_validator/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        t.ajax.reload();
    ...
    $('#mainTable').on('click', 'a.btnedit', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#groupvalidatorname").val(d.groupvalidatorname).focus();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'user/group_validator/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTbma(t, 'user/group_validator/delete');
