...
class Libauth {
    ...
    function login($user, $pass) {
        ...
        if (is_array($ses)) {
            ...
            if ($this->db->insert('login_log', $data)) {
                $this->db->update('user',array('lastlog'=>date('Y-m-d H:i:s', time())),array('id'=>$ses['id']));
    ...
    function _login($user, $pass) {
        ...
        $get = $this->db->get('v_user')->row_array();
    ...
    function _getUserRoles($userGroup) {
        $data = $this->db->select('controller,method')
                ->get_where('group_role', array('group_id' => $userGroup, 'status' => '1'))
                ->result_array();
        $r = array();
        foreach ($data as $val) {
